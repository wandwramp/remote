...
static char *pfix_home(s) char *s;
{
	static char buf[256];
...
void read_parms()
{
	...
	mfcolor = Jcolor(P_MFG);
	mbcolor = Jcolor(P_MBG);
	tfcolor = Jcolor(P_TFG);
	tbcolor = Jcolor(P_TBG);
	sfcolor = Jcolor(P_SFG);
	sbcolor = Jcolor(P_SBG);
 ...
 */
static void psets(s, w) char *s, *w;
{
	struct pars *p = (struct pars *)s;
 ...
 */
void get_bbp(ba, bi, pa, curr_ok) char *ba;
char *bi;
char *pa;
int curr_ok;
{
